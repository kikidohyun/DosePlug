...
from dose_plug import DosePlugModule
...
class DeformConv2d(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None, modulation=False):
        ...
        self.p_conv.weight.register_hook(lambda g: g * 0.1)
        if self.p_conv.bias is not None:
            self.p_conv.bias.register_hook(lambda g: g * 0.1)
        ...
        if modulation:
            ...
            if modulation:
                self.m_conv.weight.register_hook(lambda g: g * 0.1)
                if self.m_conv.bias is not None:
                    self.m_conv.bias.register_hook(lambda g: g * 0.1)
...
class Fusion_conv(nn.Module):
    def __init__(self, in_ch, out_ch, max_sinogram_libraries: int = 0, use_dose_plug: bool = False):
        super(Fusion_conv, self).__init__()
        self.use_dose_plug = bool(use_dose_plug)
        self.dose_plug = DosePlugModule(
            feature_channels=in_ch,
            num_sinogram_libraries=31,
            sinogram_library_size=16,
            sinogram_library_bank_path=None,
        )
        max_sinogram_libraries = int(max_sinogram_libraries or 0)
        if max_sinogram_libraries > 0:
            cur_sinogram_libraries = int(self.dose_plug.sinogram_library_bank.shape[1])
            if max_sinogram_libraries < cur_sinogram_libraries:
                trimmed = self.dose_plug.sinogram_library_bank[:, :max_sinogram_libraries].detach().clone()
                self.dose_plug.sinogram_library_bank = nn.Parameter(trimmed, requires_grad=self.dose_plug.sinogram_library_bank.requires_grad)
                if hasattr(self.dose_plug, "sinogram_library_attn"):
                    self.dose_plug.sinogram_library_attn.num_sinogram_libraries = max_sinogram_libraries

        self.dose_plug.sinogram_library_bank.requires_grad_(False)
        self.last_pred_dose = None
    ...
    def forward(self, x, y, x_input=None):
        fusion = torch.cat((x, y), dim=1)
        if self.use_dose_plug and (x_input is not None):
            fusion, pred_dose, attn_score = self.dose_plug(x_input, fusion)
            self.last_pred_dose = pred_dose
...
class BioInspiredLittleAddInhibitionBlock(nn.Module):
    def __init__(self, in_channel, inner_channel, out_channel, strides=1):
        ...
        self.MidgetCells = nn.Sequential(
            nn.Conv2d(in_channel, inner_channel, kernel_size=3, stride=strides, padding=1),
            nn.ReLU(inplace=True),
...
class DPN(nn.Module):
    def __init__(self, max_sinogram_libraries: int = 0, use_dose_plug: bool = False):
        ...
        self.fusion = Fusion_conv(
            64, 16,
            max_sinogram_libraries=max_sinogram_libraries,
            use_dose_plug=use_dose_plug,
        )
    ...
    def forward(self, x):
        ...
        out = self.fusion(sa3, ventralconv3, x_input=x)
